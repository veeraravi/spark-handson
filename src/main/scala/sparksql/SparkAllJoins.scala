...
//col("dfperson.personid") === col("dfprofile.personid")
 ...
 //     ,Seq("personid")
    ...
    joined_df.select(col("dfperson.personid"),
      col("dfperson.name")
      , col("dfperson.age")
    ...
    joinTypes.zipWithIndex.foreach { case (joinType, index) =>
      ...
      df_asPerson.join(df_asProfile,  Seq("personid"), joinType)
        .orderBy("personid")
        .show()
    }
  ...
  """.stripMargin)
    ...
    println(" ======== df_asPerson ====== ")
    df_asPerson.show()
    val dinstinctPersion = df_asPerson.dropDuplicates(Seq("personid"))
    dinstinctPersion.show()
    println(" ======== df_asProfile ====== ")
    df_asProfile.show()

    println("Case 5.1 df_asPerson.except(df_asProfile) Except demo")

    dinstinctPersion.except(df_asProfile).show
